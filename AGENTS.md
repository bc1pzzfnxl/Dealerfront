...
- **Balance pass (P31)**: the simulation was **not viable** — every cartel went bankrupt and games never ended. Seven root causes, all fixed:
  1. **Bootstrap order inverted**: `ECONOMY_CHAIN` was `lab → storefront → front`. A Lab with no Storefront piles up unsellable Product → dead money → no second building. Now **`storefront` first** (it earns immediately through the external supplier), and while the chain is incomplete the AI **saves** (`chainIncomplete`) instead of spamming Housing (paid in Members, always affordable).
  2. **Zombie factions**: `eliminated` was only set by assaults and police liquidations, so a faction zeroed by **encirclement** lingered forever as an inert zombie. Now decided by the map in `recount()`.
  3. **Pressure ratchet** (see below).
  4. **AI cadence**: every non-combat action in `think()` used to `continue`, so an AI that built a Lab never attacked. Actions are now **additive**, and the AI pushes **2 fronts** per decision like the balancing bot.
  5. **Two dead cartels**: the Paris map ships **4 spawns** but the game plays **6** factions, and `spawns[index]` was `undefined` for the extras — they started with **zero** quarter and never acted. Spawns are now **derived** (farthest built quarters from the ones already taken). The generator default was also raised to 6.
  6. **Clean cash had no sink**: the AI spent Clean cash only on tech, strikes and corruption, ending games with **2–4 M unspent**. `aiSpend` now buys **armament**, **quarter buyouts**, **contracts** and **mercenaries** (surplus Dirty → Members). Clean cash held at the end fell from 2–4 M to **11–243 k**.
